   ...
   virtual void update() 
     {
      ...
      if(Time[0]-entryTime>=60*(context.getLengthIn1MBarsOfWaitingPeriod()+1)) 
        {
         if((rangeHigh-rangeLow)>((context.getMaxTollarableATRPercentageForLimitOrder()/100.00)*context.getATR())) 
           {
...
public: 
   SellLimitOrderOpened(Trade *aContext,double aCancelLevel) : TradeState(aContext) {
...
public: 
   BuyLimitOrderOpened(Trade *aContext,double aCancelLevel) : TradeState(aContext) {
      this.cancelLevel = aCancelLevel; 
