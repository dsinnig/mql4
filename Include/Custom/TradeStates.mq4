...
/*class LowestLowReceivedEstablishingEligibilityRange : public TradeState 
  {
public:
     LowestLowReceivedEstablishingEligibilityRange(Trade *aContext):TradeState(aContext) 
     {
      ...
      if(Time[0]-entryTime>=60*(context.getLengthIn1MBarsOfWaitingPeriod()+1)) 
        {
         if((rangeHigh-rangeLow)>((context.getPercentageOfATRForMaxVolatility()/100.00)*context.getATR())) 
           {
            //Print ("MaxPer: ", context.getPercentageOfATRForMaxRisk());
            Print("Range to high for limit or stop entry. Range high: ",rangeHigh," Range low: ",rangeLow);
         ...
         //Stop buy entry  
           ...
           {
            Print("Range eligible for BuyStop entry. Range high: ",rangeHigh," Range low: ",rangeLow," Range: ",int((rangeHigh-rangeLow)*100000));
           ...
           {
            Print("Range eligible for Buy Limit Entry. Range high: ",rangeHigh," Range low: ",rangeLow," Range: ",int((rangeHigh-rangeLow)*100000)); 
            Print("Limit entry: ", NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits), " Stop loss: ", NormalizeDouble(rangeLow - 0.00020, Digits), "Current Bit: ", Bid);
            
            int ticket;
            
            if (Ask > NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits))
               ticket = OrderSend(Symbol(), OP_BUYLIMIT, 1, NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits), 4, NormalizeDouble(rangeLow - 0.00020, Digits), 0, "BuyLimitOrder", 0, 0, clrBlue);
            else
               ticket = OrderSend(Symbol(), OP_BUYSTOP, 1, NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits), 4, NormalizeDouble(rangeLow - 0.00020, Digits), 0, "BuyLimitOrder", 0, 0, clrBlue);
            ...
            if(result==NO_ERROR) 
              {
               context.setOrderTicket(ticket);
               context.setStopLoss(NormalizeDouble(rangeLow - 0.00020, Digits));
               context.setPlannedEntry(NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits));
               //Parametrize the 4
                context.setInitialProfitTarget (NormalizeDouble(context.getPlannedEntry() + ((context.getPlannedEntry() - context.getStopLoss()) * (context.getPercentageOfATRForMinProfitTarget() / 10)), Digits));
               //up tp 20% of ATR
               context.setState(new BuyLimitOrderOpened(context, (rangeLow + context.getATR() * 0.2)));
               
               Print("Buy order  placed @",NormalizeDouble(rangeLow + context.getATR() * 0.1, Digits)," Ticket number: ",ticket);
               delete GetPointer(this);
              }
            if((result==RETRIABLE_ERROR) && (ticket==-1)) 
              {
...
class HighestHighReceivedWaitingEstablishingTradingChannel : public TradeState 
  {
public:
      HighestHighReceivedWaitingEstablishingTradingChannel(ATRTrade* aContext) 
     {
      this.context = aContext;
      this.barsElapsedSinceLowestLow=0;
      this.entryTime = Time[0];
      this.rangeHigh = -1;
      this.rangeLow=99999;
      context.addLogEntry("Highest high found - establishing eligibility range. Highest high: " + DoubleToString(Close[0]), true); 
      }
         ...
         if((rangeHigh-rangeLow)>((context.getPercentageOfATRForMaxVolatility()/100.00)*context.getATR())) 
           {
            ...
            string reason="Range too big both Stop Entry and Limit Entry"+DoubleToStr(rangeHigh-rangeLow)+" - "+IntegerToString((int)((rangeHigh-rangeLow)*100000))+" micro pips)";
           ...
           {           
            ...
            Print("Range eligible for Sell Stop Entry. Range high: ",rangeHigh," Range low: ",rangeLow," Range: ",int((rangeHigh-rangeLow)*100000));
            //Print ("Trying to open order with: Entry: ", NormalizeDouble(minHigh + 0.00020, Digits), "Stop Loss: ", NormalizeDouble(minLow - 0.00020, Digits));
           ...
           //Limit Order Entry
           {
            Print("Range eligible for Sell Limit Entry. Range high: ",rangeHigh," Range low: ",rangeLow," Range: ",int((rangeHigh-rangeLow)*100000)); 
            Print("Limit entry: ", NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits), " Stop loss: ", NormalizeDouble(rangeHigh + 0.00020, Digits), "Current Bit: ", Bid);
            
            int ticket;
            
            if (Bid < NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits))
               ticket = OrderSend(Symbol(), OP_SELLLIMIT, 1, NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits), 4, NormalizeDouble(rangeHigh + 0.00020, Digits), 0, "SellLimitOrder", 0, 0, clrBlue);
            else
               ticket = OrderSend(Symbol(), OP_SELLSTOP, 1, NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits), 4, NormalizeDouble(rangeHigh + 0.00020, Digits), 0, "SellLimitOrder", 0, 0, clrBlue);
            ...
            if(result==NO_ERROR) 
              {
               context.setOrderTicket(ticket);
               context.setStopLoss(NormalizeDouble(rangeHigh + 0.00020, Digits));
               context.setPlannedEntry(NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits));
               //Parametrize the 4
               context.setInitialProfitTarget (NormalizeDouble(context.getPlannedEntry() - ((context.getStopLoss() - context.getPlannedEntry()) * (context.getPercentageOfATRForMinProfitTarget() / 10)), Digits));
               //up tp 20% of ATR
               context.setState(new SellLimitOrderOpened(context, (rangeHigh - context.getATR() * 0.2)));
               

               Print("Sell order placed @",NormalizeDouble(rangeHigh - context.getATR() * 0.1, Digits)," Ticket number: ",ticket);
               delete GetPointer(this);
              }
            if((result==RETRIABLE_ERROR) && (ticket==-1)) 
              {
      ...
      if(Bid < cancelLevel) 
        {
         ...
         context.setState(new TradeClosed(context, "Price went below 20% of ATR"));
         delete GetPointer(this);
         return;
      ...
      if(Ask > cancelLevel) 
        {
         ...
         ErrorManager::analzeAndProcessResult();
         //error checks missing
         context.setState(new TradeClosed(context, "Price went above 20% of ATR"));
         delete GetPointer(this);
         return;
...
class TradeClosed : public TradeState 
  {
public:
                     TradeClosed(ATRTrade* aContext, string aReason) 
     {
      this.reason=aReason;
      this.context = aContext;
...
class ProfitTargetReachedLookForInitialLowestLow : public TradeState 
  {
public:
                     ProfitTargetReachedLookForInitialLowestLow(ATRTrade* aContext, datetime _timeWhenProfitTargetWasReached)
     {
      ///Is time parameter reakky needed, why not just get the time when the constructor is called?
...
class ProfitTargetReachedLookForInitialHighestHigh : public TradeState 
  {
public:
                     ProfitTargetReachedLookForInitialHighestHigh(ATRTrade* aContext, datetime _timeWhenProfitTargetWasReached)
     {
